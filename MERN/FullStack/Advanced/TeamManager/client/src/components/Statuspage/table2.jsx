...
export default function StatusTable({ players, setPlayers }) {
    ...
    const updateStatus = (playerId, newStatus) => {
        const updatedPlayer = players.map(player => 
            player._id === playerId ? { ...player, gameTwoStatus: newStatus } : player
        );
        
        setPlayers(updatedPlayer);

        axios.patch(`http://localhost:8000/api/teammanager/${playerId}`, { gameTwoStatus: newStatus })
          .then((res) => {
              console.log(res);
          })
          .catch(err => {
...
: buttonStatus === 'Not Playing' ? { backgroundColor: 'red', color: 'white' }: { backgroundColor: 'gold', color: 'white' };
    ...
    return (
        <div className='border-table'>
            <h2><Link to={"/status/game/1"} >Game 1</Link> | <Link to={"/status/game/2"} className='underline-link'>Game 2</Link> | <Link to={"/status/game/3"}>Game 3</Link></h2>
                    ...
                    <TableBody>
                        {players.map((eachPlayer) => (
                            <TableRow key={eachPlayer._id}>
                                ...
                                <TableCell>
                                    <Button style={getButtonStyle(eachPlayer.gameTwoStatus, 'Playing')}
                                            onClick={() => updateStatus(eachPlayer._id, 'Playing')}>
                                        Playing
                                    </Button>
                                    <Button style={getButtonStyle(eachPlayer.gameTwoStatus, 'Not Playing')}
                                            onClick={() => updateStatus(eachPlayer._id, 'Not Playing')}>
                                        Not Playing
                                    </Button>
                                    <Button style={getButtonStyle(eachPlayer.gameTwoStatus, 'Undecided')}
